<div x-data="wireui_timepicker({
    ...
    <div class="fixed inset-0 z-10 md:absolute md:inset-auto md:top-0 md:right-0 md:mt-6"
        x-cloak
        x-show="showPicker"
        x-on:click.outside="closePicker"
        x-on:keydown.escape.window="closePicker"
        wire:ignore>
        <div class="flex items-end justify-center h-screen md:h-48"
            style="min-height: -webkit-fill-available; min-height: fill-available;">
            <div class="fixed inset-0 bg-secondary-500 bg-opacity-75 transition-opacity md:hidden
            ...
            <div class="w-full rounded-t-md p-2.5 border border-secondary-200 bg-white transition-all
                        relative md:rounded-lg md:shadow-md md:w-48 dark:bg-secondary-800 dark:border-secondary-600"
                x-show="showPicker"
                tabindex="-1"
                x-transition:enter="ease-out duration-300"
                ...
                <button type="button" class="cursor-pointer hidden md:flex absolute top-2 right-2 focus:outline-none" x-on:click="closePicker">
                    <x-dynamic-component
                        :component="WireUiComponent::resolve('icon')"
                ...
                <ul class="mt-1 w-full h-64 md:h-32 pb-1 pt-2 overflow-y-auto">
